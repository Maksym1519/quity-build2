...
const LastInvestitionPosts = () => {
  ...
  const dataFromRedux = useAppSelector(blogInfoInvestitionFromServer);
          ...
          {cryptoNewsArray.slice(0, 4).map((item, index) => (
            <div className={l.item} key={index}>
              <div className={l.category__wrapper}>
                ...
                <p className={l.category}>{item.attributes.blogInvestitionCategory}</p>
              </div>
              <h3 className={l.title}>{item.attributes.blogInvestitionTitle}</h3>
...
export default LastInvestitionPosts;
