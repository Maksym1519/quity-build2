...
const LastCryptoPosts = () => {
  ...
  const dataFromRedux = useAppSelector(blogInfoFromServer);
          ...
          {cryptoNewsArray.slice(0, 4).map((item, index) => (
            <div className={l.item} key={index}>
              <div className={l.category__wrapper}>
                ...
                <p className={l.category}>{item.attributes.blogCategory}</p>
              </div>
              <h3 className={l.title}>{item.attributes.blogTitle}</h3>
...
export default LastCryptoPosts;
