...
const LastEquipmentPosts = () => {
  ...
  const dataFromRedux = useAppSelector(blogInfoEquipmentFromServer);
          ...
          {cryptoNewsArray.slice(0, 4).map((item, index) => (
            <div className={l.item} key={index}>
              <div className={l.category__wrapper}>
                ...
                <p className={l.category}>{item.attributes.blogEquipmentCategory}</p>
              </div>
              <h3 className={l.title}>{item.attributes.blogEquipmentTitle}</h3>
...
export default LastEquipmentPosts;
