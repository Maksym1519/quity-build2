...
import { presenceInfo } from "./catalog/filtrationSlice";
import { newInfo } from "./catalog/filtrationSlice";
...
export const AsicData = () => {
  ...
  const presenceState = useAppSelector(presenceInfo);
  const newState = useAppSelector(newInfo)
  ...
  async function getAsicInfo() {
    try {
      ...
      // if (presenceState && presenceState === true) {
      //   const presenceArray = dataResponse.filter(
      //     (item) => item.attributes.presence === "В наличии"
      //   );
      //   setAsic(presenceArray);
      //   setAsicImages(dataResponseImages)
      // } else if (newState && newState === true) {
      //   const newArray = dataResponse.filter((item) => item.attributes.condition === "new")
      //   setAsic(newArray)
      // }
      // else {
