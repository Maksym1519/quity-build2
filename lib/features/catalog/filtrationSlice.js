...
export const { setPresence, setFilteredArray, setNew, setUsed, setPrice, setHashRate, setProfit, setPower, setMaker, setAlgorithm } =
...
export const algorithmInfo = (state) => state.filtration.algorithm
...
export const FiltrationPresenceData = () => {
  ...
  const algorithmFromFilter = useAppSelector(algorithmInfo)
  ...
  useEffect(() => {
    try {
      ...
      if (hashRateFromFilter !== null && filteredArray !== undefined) {
        filteredArray = filteredArray.filter(
          (item) => item.attributes.ths >= hashRateFromFilter[0] && item.attributes.ths <= hashRateFromFilter[1]
        );
      }
      // Фильтрация по доходности
      if (profitFromFilter !== null && filteredArray !== undefined) {
        filteredArray = filteredArray.filter(
          (item) => item.attributes.profit >= profitFromFilter[0] && item.attributes.profit <= profitFromFilter[1]
        );
      }
      // Фильтрация по энергопотреблению
      if (powerFromFilter !== null && filteredArray !== undefined) {
        filteredArray = filteredArray.filter(
          (item) => item.attributes.w >= powerFromFilter[0] && item.attributes.w <= powerFromFilter[1]
        );
      }
      // Фильтрация по производителю
      if (makerFromFilter && makerFromFilter.length > 0 && filteredArray !== undefined) {
        filteredArray = filteredArray.filter((item) =>
          makerFromFilter.includes(item.attributes.maker)
        );
      }
      // Фильтрация по хешу
      if (algorithmFromFilter && algorithmFromFilter.length > 0 && filteredArray !== undefined) {
         filteredArray = filteredArray.filter((item) => algorithmFromFilter.includes(item.attributes.algorithm))
      }
