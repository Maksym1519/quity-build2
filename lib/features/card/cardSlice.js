    ...
    reducers: {
        ...
        addToBucket: (state, action) => {
            ...
            payloadArray.forEach(item => {
              ...
              if (existingItem) {
                existingItem.quantity += 1; // если товар уже в корзине, увеличиваем количество
              } else {
                updatedBucket.push({ ...item, quantity: 1 }); // добавляем товар с количеством 1
              }
            });
...
export const {setInfo,setCard,setCatalog,setAdditionalFeatures,setPayment,setFeedback,setDataBucket,addToBucket,removeFromBucket,setClickBucket,setBucketLength} = cardSlice.actions;
