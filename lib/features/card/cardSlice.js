  ...
  reducers: {
    ...
    addToBucket: (state, action) => {
      ...
      payloadArray.forEach((item) => {
        const existingItem = updatedBucket.find(
        ...
        if (existingItem) {
          existingItem.quantity =
            action.payload.counter > 0
              ? existingItem.quantity + action.payload.counter
              : existingItem.quantity - 1;

          if (existingItem.quantity <= 0) {
            const indexToRemove = updatedBucket.findIndex(
              (bucketItem) => bucketItem.id === item.id
            );
    ...
    updateBucket: (state, action) => {
      const updatedBucket = state.bucketGoods.map((bucketItem) =>
        bucketItem.id === action.payload.id
          ? { ...bucketItem, quantity: Math.max(1, action.payload.quantity), ...action.payload }
          : bucketItem
      );
