  ...
  reducers: {
    ...
    addToBucket: (state, action) => {
      ...
      if (existingUserBucket) {
        ...
        // Обновляем товары в этой корзине
        payloadArray.forEach((item) => {
          ...
          if (existingItem) {
            existingItem.quantity +=
              action.payload.counter > 0
                ? action.payload.counter
                : -1;

            if (existingItem.quantity <= 0) {
              existingUserBucket.userGoods = existingUserBucket.userGoods.filter(
        ...
        // Если корзина для пользователя не существует
...
//-------------------------------------------------------------------
    ...
    removeFromBucket: (state, action) => {
      ...
      const userId = localStorage.getItem("id");
      const userBucket = state.bucketGoods.find((userBucket) => userBucket.id === userId);
       if (userBucket) {
    ...
    updateBucket: (state, action) => {
      const updatedBucket = state.bucketGoods.map((bucketItem) =>
        bucketItem.id === action.payload.id
          ? {
              ...bucketItem,
      ...
      // Обновляем общую сумму
      const totalSum = updatedBucket.reduce(
